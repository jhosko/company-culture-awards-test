...
export default function Admin() {
  ...
  const verifiedBallots = useMemo(
    () =>
      ballots.filter(
        b => b.verification_status === "verified" && b.review_status !== "excluded"
      ),
    [ballots]
  );
  ...
  async function loadReporting(token) {
    ...
    if (!ballotResponse.ok) {
      throw new Error(ballotResult?.error || "Could not load reporting.");
    }

    if (!newsletterResponse.ok) {
      throw new Error(
        newsletterResult?.error || "Could not load newsletter signups."
      );
    }
  ...
  async function loadSponsors(token) {
    ...
    if (!response.ok) {
      throw new Error(result?.error || "Could not load sponsors.");
    }
  ...
  async function deleteSponsor(id) {
    ...
    try {
      ...
      setError(err.message || "Could not remove sponsor.");
    }
  }
                ...
                {sponsors.map(sponsor => (
                  <article className="sponsorAdminCard" key={sponsor.id}>
                    ...
                    <div className="sponsorAdminActions">
                      <button type="button" onClick={() => editSponsor(sponsor)}>
                        Edit
                      </button>
                      <button type="button" onClick={() => deleteSponsor(sponsor.id)}>
                        Remove
                      </button>
                    </div>
                  </article>
              ...
              <form id="sponsor-editor-form" className="sponsorEditor" onSubmit={saveSponsor}>
                <div className="cmsField">
                  ...
                  <input
                    ...
                    onChange={e =>
                      setSponsorDraft({ ...sponsorDraft, name: e.target.value })
                    }
                  />
                </div>
                ...
                <div className="cmsField sponsorUploadField">
                  ...
                  <div className="sponsorUploadBox">
                    <input
                      ...
                      onChange={e =>
                        selectLogoFile(e.target.files?.[0] || null)
                      }
                    />
            ...
            <>
              ...
              >
                <button
                  className="button"
                  onClick={() =>
                    downloadCsv(ballots, "company-culture-ballots.csv")
                  }
                >
                  Export Ballots CSV
                </button>
                ...
                <button
                  ...
                  onClick={() =>
                    downloadCsv(
                      newsletterSignups,
                      "company-culture-newsletter-signups.csv"
                    )
                  }
